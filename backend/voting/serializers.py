...
from .models import AdminMFA, Candidate, Election, Vote
...
class EncryptedVoteCreateSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        voter_hash = hashlib.sha256(attrs["voter_identifier"].strip().lower().encode("utf-8")).hexdigest()
...
class RegisterSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if attrs["role"] == "admin":
            ...
            expected = getattr(settings, "ADMIN_INVITE_KEY", "")
            if not expected or attrs.get("admin_invite_key", "") != expected:
                raise serializers.ValidationError("Invalid admin invite key.")
